...
{
    /// <summary>
    /// This controller is for exporting RadzenDataGrid data
    /// </summary>
    /// <remarks>This file contains shared code.  Place code for specific data grids in other files.
    /// (E.G. ExportController.Agency.cs for agency grids)</remarks>
    [ApiController]
    [Microsoft.AspNetCore.Mvc.Route("[controller]")]
    [Authorize]
    public partial class ExportController(IDataAccess dataAccess, ILoggingService loggingService)
        : Controller
    {
        [Inject] public required IDataAccess DataAccess { get; set; } = dataAccess;
        [Inject] public required ILoggingService LoggingService { get; set; } = loggingService;

        public IQueryable ApplyQuery<T>(IQueryable<T> items, IQueryCollection query) where T : class
        {
        ...
        {
            var columns = GetProperties(query.ElementType).ToArray();
            ...
            //Add column headers and output the file
            var result = new FileStreamResult(new MemoryStream(Encoding.Default.GetBytes($"{string.Join(",", columns.Select(c => c.Key.Replace(ExportColumnSubstitution.SpaceSubstitution, ' ')))}{Environment.NewLine}{sb}")), "text/csv");
            result.FileDownloadName = Path.ChangeExtension(!string.IsNullOrEmpty(fileName) ? fileName : "Export", "csv");
        ...
        public FileStreamResult ToExcel(IQueryable query, string? fileName = null)
        {
            ...
            using (var document = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
            {
                ...
                var sheet = new Sheet() { Id = workbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "Exported Data" };
                sheets.Append(sheet);
                ...
                foreach (var column in columns)
                {
                    ...
                    {
                        CellValue = new CellValue(column.Key.Replace(ExportColumnSubstitution.SpaceSubstitution, ' ')),
        ...
        {
            //TODO:Consider caching the property infos for types for performance
            return target.GetType().GetProperty(name)?.GetValue(target);
        }

        /// <summary>
        /// Gets the column names and type for a type
        /// </summary>
        /// <param name="type">Type of the data being exported</param>
        /// <returns>List of Column names and types</returns>
        /// <remarks>Removes properties that do not serialize into human-readable text</remarks>
        ...
        /// <summary>
        /// Data types of columns that can exported
        /// </summary>
        /// <remarks>If a column is being excluded from the output, it's type may need to be added to this list.  Only types that serialize to human-readable text should be included.</remarks>
        private static readonly HashSet<Type> _allowedSimpleTypes = [typeof(Guid), typeof(DateOnly), typeof(DateTime)];

        /// <summary>
        /// Determines if the data in the column is suitable to be outputted to an export file
        /// </summary>
        /// <param name="type">Type of the column</param>
        /// <returns>True for types that can be outputted as text, False for other objects</returns>
        public static bool IsSimpleType(Type type)
        {
            ...
            if (_allowedSimpleTypes.Contains(underlyingType))
                return true;
