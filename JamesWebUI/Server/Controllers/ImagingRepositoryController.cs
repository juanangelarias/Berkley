...
namespace JamesWebUI.Server.Controllers
{
    ...
    public class ImagingRepositoryController(ServerImagingAccess imagingAccess, ILoggingService loggingService)
    {
        ...
        {
            var fileType = description.Split('.').Last();
        ...
        /// <returns>StatusCodeResult</returns>
        [HttpPost("UploadDocuments/{docTypes}/{category:int}")]
        public async Task<StatusCodeResult> UploadDocuments(IFormFileCollection uploadedFiles, string docTypes, int category, [FromQuery] string batch = "", [FromQuery] string descriptions = "")
        {
            //Pre-validate
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(batch))
                    ...
                    batch = $"James{category}-{uploadedFiles.Count}-{DateTime.Now.ToShortDateString()}";
                var options = new ParallelOptions() { MaxDegreeOfParallelism = 4 };
                var allTypes = docTypes.Split(':');
                var allFilenames = descriptions.Split(':');
                if (uploadedFiles.Count != allTypes.Length || uploadedFiles.Count != allFilenames.Length)
                {
                    loggingService.LogError("Invalid descriptions and/or document types error",
                        ...
                        new Dictionary<string, string>{{"Types submitted", allTypes.Length.ToString()},{"Filenames submitted",allFilenames.Length.ToString()}, {"Uploaded files", uploadedFiles.Count.ToString()}});
                    return new StatusCodeResult(422);//Unprocessable content
                }
                var docsWithInfo = Enumerable.Range(0, uploadedFiles.Count - 1).Select(i => new DocumentTypeFileName
                    { Filename = allFilenames[i], DocumentType = allTypes[i], UploadedFile = uploadedFiles[i] });

                await Parallel.ForEachAsync(docsWithInfo, options, async (uf, ct) => await UploadDocument(uf.UploadedFile, uf.DocumentType, category, batch));
