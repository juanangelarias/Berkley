...
namespace JamesWebUI.Server.Controllers
{
    ...
    public class ImagingRepositoryController(ServerImagingAccess imagingAccess, ILoggingService loggingService)
    {
        ...
        public async Task<ActionResult> UploadDocument(IFormFile file, string docType, int category, string id, [FromQuery] string batch = "", [FromQuery] string description = "")
        {
        ...
        public async Task<StatusCodeResult> UploadDocuments(IFormFileCollection files, string docTypes, int category, string id, [FromQuery] string batch = "", [FromQuery] string descriptions = "")
        {
            //Pre-validate
            ...
            try
            {
                ...
                await Parallel.ForEachAsync(docsWithInfo, options, async (uf, ct) => await UploadDocument(uf.UploadedFile, uf.DocumentType, category, id, batch, uf.Description));
